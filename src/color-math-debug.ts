export function spectraToXyz(spectra: Float64Array): [number, number, number] {
  ...
  const norm = S.reduce((s, v) => s + v * CMF_Y[CMF_Y.indexOf(Math.max(...CMF_Y))], 0);
...
function labF(t: number): number {
  const delta = 6 / 29;
  return t > delta **___ 3 ? Math.cbrt(t) : t / (3 * delta **___ 2) + 4 / 29;
}
...
export function deltaE00(lab1: [number, number, number], lab2: [number, number, number]): number {
  ...
  const G = 0.5 * (1 - Math.sqrt(avgC **___ 7 / (avgC **___ 7 + 25 **___ 7)));
  const a1p = a1 * (1 + G);
  const a2p = a2 * (1 + G);
  ...
  const SL = 1 + 0.015 * (avgL - 50) **___ 2 / Math.sqrt(20 + (avgL - 50) **___ 2);
  const SC = 1 + 0.045 * avgCp;
  const SH = 1 + 0.015 * avgCp * T;

  const deltaTheta = 30 * Math.exp(-((avgHp - 275) / 25) **___ 2);
  const RC = 2 * Math.sqrt(avgCp **___ 7 / (avgCp **___ 7 + 25 **___ 7));
